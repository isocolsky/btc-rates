...
Rates.prototype._dump = function(cb) {
  var self = this;

  var all = [];
  self.db.readStream()
    .on('data', function (data) { all.push(data); })
...
Rates.prototype.getRates = function(sourceId, code, tsList, cb) {
	var self = this;

	preconditions.shouldBeArray(tsList);

	async.map(tsList, function (ts, cb) {
		self.getRate(sourceId, code, ts, function (err, rate) {
			if (err) return cb(err);
			return cb(null, { ts: ts, rate: rate });
		});
	}, function (err, res) {
		if (err) return cb(err);
		return cb(null, res);
