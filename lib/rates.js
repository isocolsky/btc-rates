...
Rates.prototype._dump = function(opts) {
  var self = this;

  var all = [];
  self.db.readStream(opts)
    .on('data', console.log);
};
...
Rates.prototype._getOneRate = function(sourceId, code, ts, cb) {
  var self = this;
...
Rates.prototype.getRate = function(sourceId, code, ts, cb) {
  ...
  if (!_.isArray(ts)) {
  	return self._getOneRate(sourceId, code, ts, function (err, rate) {
  		if (err) return cb(err);
  		return cb(null, { rate: rate });
  	});
  }

  async.map(ts, function(ts, cb) {
    self._getOneRate(sourceId, code, ts, function(err, rate) {
      if (err) return cb(err);
      return cb(null, {
        ts: parseInt(ts),
        rate: rate
